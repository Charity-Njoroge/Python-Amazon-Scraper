"""
python script to Scrape Amazon Product Data and Pricing
from a single amazon page.
"""
...
for url in url_array:
    asin_array.append(url[url.find(start)+len(start):url.rfind(end)])  # this
    # array has all the asin numbers

headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:66.0)"
...
for asin in asin_array:
    ...
    #  structure of a url
    response = session.get(amazon_url, headers=headers, verify=False)  # get
    # the response
    item_array.append(
        response.html.search('a-color-price">${}<')[0])  # Extracting the price
    # Extracting the text containing the product details
    details = (response.html.search
               ('P.when("ReplacementPartsBulletLoader").execute(function'
                '(module){ module.initializeDPX(); }){}</ul>;')[0])

    details_arr = []  # Declaring an array to store individual details
    details = re.sub("\n|\r", "", details)  # Separate the details from text
    details_arr = re.findall(r'\>(.*?)\<', details)  # Store details in
    #  the array.
    for i, row in enumerate(details_arr):
        details_arr[i] = row.replace("\t", "")  # Remove tabs from details
    details_arr = list(
        filter(lambda a: a != '', details_arr))  # Remove empty spaces.
    details_arr = [row.strip() for row in
                   details_arr]  # Remove trailing and starting spaces.
